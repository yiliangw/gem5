...
CxlMemory::CxlResponsePort::CxlResponsePort(const std::string &_name,
                                            ...
                                            Cycles _protoProcLat,
                                            int _resp_limit,
                                            AddrRange _cxlMemRange)
    : ResponsePort(_name), cxlMemory(_cxlMemory), memReqPort(_memReqPort),
      protoProcLat(_protoProcLat), cxlMemRange(_cxlMemRange),
...
CxlMemory::CxlRequestPort::CxlRequestPort(const std::string &_name,
    ...
    : RequestPort(_name), cxlMemory(_cxlMemory), cxlRspPort(_cxlRspPort),
...
CxlMemory::CXLCtrlStats::CXLCtrlStats(CxlMemory &_cxlMemory)
    : statistics::Group(&_cxlMemory),
      ...
      ADD_STAT(memToCXLCtrlRsp, "Distribution of the time intervals between "
                                "consecutive mem responses from the memory "
                                "media to the CXLCtrl (Cycle)")
{
    reqQueueLenDist.init(0, 49, 10).flags(statistics::nozero);
    rspQueueLenDist.init(0, 49, 10).flags(statistics::nozero);
    rspOutStandDist.init(0, 49, 10).flags(statistics::nozero);
    reqQueueLatDist.init(12000, 41999, 1000).flags(statistics::nozero);
    rspQueueLatDist.init(12000, 41999, 1000).flags(statistics::nozero);
    memToCXLCtrlRsp.init(0, 299, 10).flags(statistics::nozero);
}
...
{
    return PciEndpoint::getAddrRanges();
}
...
CxlMemory::CxlRequestPort::recvTimingResp(PacketPtr pkt)
{
    ...
    if (cxlMemory.preRspTick == -1) {
        ...
        cxlMemory.stats.memToCXLCtrlRsp.sample(cxlMemory.ticksToCycles(
            cxlMemory.clockEdge() - cxlMemory.preRspTick));
        cxlMemory.preRspTick = cxlMemory.clockEdge();
    }
...
CxlMemory::CxlRequestPort::trySendTiming()
{
    ...
    if (sendTimingReq(pkt)) {
        ...
        if (!transmitList.empty()) {
            ...
            cxlMemory.schedule(sendEvent,
                               std::max(next_req.tick, cxlMemory.clockEdge()));
        }
...
CxlMemory::CxlResponsePort::trySendTiming()
{
    ...
    if (sendTimingResp(pkt)) {
        ...
        if (!transmitList.empty()) {
            ...
            DPRINTF(CxlMemory, "Scheduling next send\n");
            cxlMemory.schedule(sendEvent,
                               std::max(next_resp.tick, cxlMemory.clockEdge()));
        }
...
CxlMemory::CxlResponsePort::recvAtomic(PacketPtr pkt)
{
    ...
    DPRINTF(CxlMemory, "access_delay=%ld, proto_proc_lat=%ld, total=%ld\n",
            access_delay, delay,
            delay * cxlMemory.clockPeriod() + access_delay);
    return delay * cxlMemory.clockPeriod() + access_delay;
}

Tick
CxlMemory::CxlResponsePort::recvAtomicBackdoor(PacketPtr pkt,
                                               MemBackdoorPtr &backdoor)
{
    Cycles delay = processCXLMem(pkt);

    return delay * cxlMemory.clockPeriod() +
           memReqPort.sendAtomicBackdoor(pkt, backdoor);
}
